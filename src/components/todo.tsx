...
interface Props {
  todo: todoType;
  changeTodoText: (id: number, title: string, text: string) => void;
...
const Todo: FC<Props> = ({
  ...
  const handleSave = async () => {
    changeTodoText(todo.id, title, text);
    setEditing(false);
  };
  ...
  return (
    <div className="flex items-center gap-2 p-4 border-gray-200 border-solid border rounded-lg">
      ...
      <input
        ...
        } outline-none read-only:border-transparent focus:border border-gray-200 rounded px-2 py-1 w-1/4`}
      />
